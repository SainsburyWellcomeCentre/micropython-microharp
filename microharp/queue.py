# queue.py: fallback Queue for MicroPython builds that omit asyncio.Queue
#
# This module is only imported when `from asyncio import Queue` raises
# ImportError (e.g. bare RP2 firmware without the full micropython-lib asyncio).
# On CPython or MicroPython ≥ 1.20 with a complete asyncio bundle the callers
# import directly from asyncio and this file is never loaded.
#
# Original implementation:
# Copyright (c) 2018-2020 Peter Hinch – MIT License
# Adapted from uasyncio V2 (Paul Sokolovsky)
...
class Queue:
    ...
    def put_nowait(self, val: int):  # Put an item into the queue without blocking.
        if self.full():
            raise QueueFull()
        self._put(val)
